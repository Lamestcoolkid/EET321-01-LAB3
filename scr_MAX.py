...
SCOPE_RESOURCE = "USB0::0xF4EC::0xEE38::SDSMMFCX6R5641::INSTR"
...
class Oscilloscope:
    ...
    def setup_measurement(self):
        ...
        """
        self.inst.write(f"PACU MAX,{SCOPE_CHANNEL}")
    ...
    def measure_on_time(self) -> float:
        ...
        """
        try:
            raw = self.inst.query(f"{SCOPE_CHANNEL}:PAVA? MAX").strip()
...
def run_sweep(arduino: Arduino, scope: Oscilloscope) -> pd.DataFrame:
    ...
    print(f"\n{'Pos':>4}  {'R_ideal (Ω)':>12}  {'Voltage (V)':>14}")
    print("-" * 36)
    ...
    for pos in range(POT_MAX_POS, POT_MIN_POS - 1, -1):
        ...
        print(f"{confirmed_pos:>4}  {r_ideal:>12.1f}  {on_time_s:>13.2f} V")
...
def plot_results(df: pd.DataFrame):
    ...
    ax1 = axes[0]
    ax1.plot(df["Position"], df["Voltage"], "o-", color="steelblue", markersize=4)
    ax1.set_xlabel("Wiper Position (0 = min, 99 = 10 kΩ)")
    ax1.set_ylabel("Max Voltage (V)")
    ax1.set_title("Max Voltage vs. Wiper Position")
    ax1.grid(True, alpha=0.4)
    ax1.invert_xaxis()  # 99→0 left-to-right mirrors the sweep

    # On-time vs. ideal resistance
    ax2 = axes[1]
    ax2.plot(df["R_ideal_ohm"], df["Voltage"], "o-", color="darkorange", markersize=4)
    ax2.set_xlabel("Ideal Resistance (Ω)")
    ax2.set_ylabel("SCR Max Voltage (V)")
