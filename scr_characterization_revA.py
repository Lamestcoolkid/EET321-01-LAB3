...
#
# USB example:  "USB0::0xF4EC::0xEE38::SDSMMFCX6R5670::INSTR"
...
class Oscilloscope:
    ...
    def capture_waveform(self) -> tuple[np.ndarray, np.ndarray] | None:
        """
        Transfer the full waveform from the scope for channel SCOPE_CHANNEL.
        Returns (time_array_s, voltage_array_v), or None on failure.
        """
        try:
            # ── 1. Fetch the waveform descriptor (scaling info) ──────────────
            self.inst.write(f"{SCOPE_CHANNEL}:WF? DESC")
            desc_raw = self.inst.read_raw()

            # Descriptor block: parse key offsets per Siglent programming guide
            # Byte offsets into the descriptor (after the #9... header):
            VGAIN_OFFSET = 156  # float32, vertical gain (V/count)
            VOFFSET_OFFSET = 159  # float32, vertical offset (V)
            HINTERVAL_OFFSET = 176  # float32, sample interval (s/point)
            HOFFSET_OFFSET = 180  # float64, horizontal offset (s)

            # Skip the ASCII block header (#9xxxxxxxxx = 11 bytes)
            header_end = desc_raw.index(b'#')
            digits = int(desc_raw[header_end + 1: header_end + 2])
            data_start = header_end + 2 + digits
            desc = desc_raw[data_start:]

            vgain = struct.unpack_from('<f', desc, VGAIN_OFFSET)[0]
            voffset = struct.unpack_from('<f', desc, VOFFSET_OFFSET)[0]
            hinterval = struct.unpack_from('<f', desc, HINTERVAL_OFFSET)[0]
            hoffset = struct.unpack_from('<d', desc, HOFFSET_OFFSET)[0]

            # ── 2. Fetch the raw sample data ─────────────────────────────────
            self.inst.write(f"{SCOPE_CHANNEL}:WF? DAT2")
            dat_raw = self.inst.read_raw()

            header_end = dat_raw.index(b'#')
            digits = int(dat_raw[header_end + 1: header_end + 2])
            data_start = header_end + 2 + digits
            raw_bytes = dat_raw[data_start:]

            # Samples are signed 8-bit integers
            samples = np.frombuffer(raw_bytes, dtype=np.int8).astype(np.float32)

            # ── 3. Convert counts → real units ───────────────────────────────
            voltages = samples * vgain - voffset
            times = np.arange(len(samples)) * hinterval + hoffset

            return times, voltages

        except Exception as e:
            print(f"  [WF] Waveform capture failed: {e}")
            return None

    def measure_on_time_from_waveform(self) -> float:
        """
        Capture the full waveform and compute SCR on-time (low duration)
        by finding contiguous regions below the 50% threshold.
        Returns on-time in seconds, or NaN on failure.
        """
        result = self.capture_waveform()
        if result is None:
            return float("nan")

        times, voltages = result

        # 50% threshold between min and max
        vmin = np.min(voltages)
        vmax = np.max(voltages)
        threshold = vmin + (vmax - vmin) * 0.5

        # Find samples where signal is below threshold (SCR conducting)
        low_mask = voltages < threshold

        # Count the longest contiguous low run (one full on-cycle)
        # Uses a diff trick to find rising/falling edges
        edges = np.diff(low_mask.astype(int))
        fall_idx = np.where(edges == 1)[0]  # low→high transitions (SCR turns off)
        rise_idx = np.where(edges == -1)[0]  # high→low transitions (SCR turns on)

        if len(fall_idx) == 0 or len(rise_idx) == 0:
            return float("nan")
            # Pair up each on→off transition and take the longest
        on_durations = []
        for r in rise_idx:
            subsequent_falls = fall_idx[fall_idx > r]
            if len(subsequent_falls) > 0:
                on_durations.append(times[subsequent_falls[0]] - times[r])

        if not on_durations:
            return float("nan")

        return float(np.max(on_durations))  # longest pulse = the real on-time
...
def list_visa_resources():
    ...
    print("\nSet oscope_RESOURCE in the CONFIG block to one of the above, then re-run.")


def run_sweep(arduino: Arduino, oscope: Oscilloscope) -> pd.DataFrame:
    """
    ...
    for pos in range(POT_MAX_POS, POT_MIN_POS - 1, -1):
        ...
        # Measure
        on_time_s = oscope.measure_on_time_from_waveform()
...
def main():
    ...
    oscope = Oscilloscope(SCOPE_RESOURCE)
    oscope.setup_measurement()
